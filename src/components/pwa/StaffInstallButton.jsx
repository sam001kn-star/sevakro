...
export default function StaffInstallButton({ className = '' }) {
  ...
  return (
    <>
      <motion.div
        ...
        exit={{ opacity: 0, y: -8 }}
        className={`flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl px-3 py-2 ${className}`}
      >
        <Smartphone className="w-4 h-4 text-blue-600 flex-shrink-0" />
        <p className="text-xs text-foreground flex-1">
          <span className="font-semibold">Install Staff App</span>
          <span className="text-muted-foreground"> for quick access</span>
        </p>
        <Button size="sm" onClick={handleClick} className="h-7 text-xs px-3 rounded-lg bg-blue-600 hover:bg-blue-700 text-white">
          {isIOS ? 'How?' : 'Install'}
        </Button>
            ...
            >
              <div className="bg-card rounded-t-3xl p-6 pb-10 shadow-2xl">
                ...
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-lg">Install Staff App on iPhone</h3>
                ...
                <Button className="w-full mt-6 bg-blue-600 hover:bg-blue-700" onClick={() => setShowIOSGuide(false)}>Got it</Button>
              </div>
            </motion.div>
          </>
        )}
